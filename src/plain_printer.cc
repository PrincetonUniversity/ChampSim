...
void champsim::plain_printer::print(O3_CPU::stats_type stats)
{
  ...
  fmt::print(stream, "\n{} cumulative IPC: {:.4g} instructions: {} cycles: {} wrong_path_insts: {} wrong_path_insts_skipped: {}\n", stats.name, std::ceil(stats.instrs()) / std::ceil(stats.cycles()),
             stats.instrs(), stats.cycles(), stats.wrong_path_insts, stats.wrong_path_skipped);
