...
void champsim::plain_printer::print(O3_CPU::stats_type stats)
{
  ...
  fmt::print(stream, "\n{} cumulative IPC: {:.4g} instructions: {} cycles: {} wrong_path_insts: {} wrong_path_insts_skipped: {} wrong_path_insts_executed: {}\n", stats.name, std::ceil(stats.instrs()) / std::ceil(stats.cycles()),
             stats.instrs(), stats.cycles(), stats.wrong_path_insts, stats.wrong_path_skipped, stats.wrong_path_insts_executed);
...
void champsim::plain_printer::print(CACHE::stats_type stats)
{
  ...
  for (std::size_t cpu = 0; cpu < NUM_CPUS; ++cpu) {
    ...
    fmt::print(stream, "{} WRONG-PATH ACCESS: {:10} LOAD: {:10} STORE: {:10} FILL: {:10} RIGHT-PATH ENTRY_EVICTED: {:10} \n", stats.name, stats.wp_load + stats.wp_store, stats.wp_load, stats.wp_store, stats.wp_fill, stats.wp_evicted);
