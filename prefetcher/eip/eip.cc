...
#define cpu intern_->cpu
#define NUM_SET intern_->NUM_SET
#define NUM_WAY intern_->NUM_WAY
#define current_cycle intern_->current_cycle
...
void eip::prefetcher_squash(uint64_t ip, uint64_t instr_id)
{
  l1i_cpu_id = cpu;
  l1i_current_cycle = current_cycle;
...
#if 1
uint32_t eip::prefetcher_cache_operate(uint64_t addr, uint64_t ip, uint8_t cache_hit, bool useful_prefetch, access_type type, uint32_t metadata_in){
  // TODO 
}
#else
...
ORIG : prefetcher_cache_fill(uint64_t addr, uint32_t set, uint32_t way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
*/
uint32_t eip::prefetcher_cache_fill(uint64_t addr, long set, long way, uint8_t prefetch, uint64_t evicted_addr, uint32_t metadata_in)
{
  l1i_cpu_id = cpu;
  l1i_current_cycle = current_cycle;
